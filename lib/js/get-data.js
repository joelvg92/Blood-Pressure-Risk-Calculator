...
FHIR.oauth2.ready().then(function(client) {
  ...
  var medRequest = new URLSearchParams();
  medRequest.set("patient", client.patient.id);
  medRequest.set("_count", 1000);
  medRequest.set("_sort", "-date");
  
  client.request("MedicationRequest?"+medRequest).then(
    function (medications) {
      medications.entry.forEach(function (medication) {
        displayMedication(medication.resource.medicationCodeableConcept.text);
      });
    }
  );
